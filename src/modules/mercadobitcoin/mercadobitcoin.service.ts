...
export class MercadoBitcoinService {
    ...
    async genericao(metodo, parametros) {
        const numeroArbitrario = Math.round(new Date().getTime() / 1000);
        let queryString = stringify({ tapi_method: metodo, tapi_nonce: numeroArbitrario });
        if (parametros) {
            queryString += '&' + stringify(parametros) + 'coin_pair=BRLBTC';
        }
        const signature = createHmac('sha512', this.config.SECRET)
